...
- `doc/runbooks/deployment.md`: complete deployment runbook covering requirements, repo structure, build process (GitHub Action → GHCR), deploy process (docker compose), data & key handling, opt-in authentication, rollback, and troubleshooting.
...
- `src/rps/crewai_runtime/config.py`: added `skills/shared/domain-glossary` to `OPERATIONAL_CREW_SKILLS` allow-list — the v0.37.17 release added it to `skills.yaml` but missed registering it here, causing a `ValueError` at crew startup for any crew that includes the glossary skill.
...
- `config/crewai/knowledge_sources.yaml`: reverted RAG bundle additions from v0.37.14 (`factual_macrocycle`, `factual_phase_recovery` and their agent/crew assignments). Investigation showed `StringKnowledgeSource` import fails silently on this platform (`lancedb` has no wheel for `macosx_26_0_x86_64`); CrewAI RAG delivery is not verifiable and must not be relied on as a content-delivery path. SKILL.md body injection is the only guaranteed delivery mechanism — content was already moved there in v0.37.14 body changes which remain.

## [0.37.14] - 2026-09-17

### Changed

- `skills/season/macrocycle-architecture/SKILL.md`: moved critical planning rules from references into the SKILL.md body (the only path guaranteed to reach the `macrocycle_architect` and `peak_window_specialist` agents). Added to **Taper rules**: minimum effective taper window (2–3 weeks including event week for ultra/brevet events > 12 hours); RELOAD→TAPER adjacency as structural **warning**, not blocker. Added new **Progressive brevet / ultra sequencing** section: preparatory brevets as stress-plus-recovery milestones, 3–7 day recovery windows, no rebuild block between final B-event and A-event taper when spacing ≤ 3 weeks. Added **Ultra/brevet durability anchors** section: kJ/kg milestone arc (20–25 / 28–32 / 35–40 kJ/kg), fueling progression (40–60 / 60–75 / 70–90 g/h), HR late-suppression as autonomic fatigue marker.
...
- `config/crewai/knowledge_sources.yaml`: added RAG bundles for macrocycle and phase-recovery references — **reverted in v0.37.15** after confirming RAG delivery is unreliable.
...
- `src/rps/orchestrator/plan_hub_worker.py`: step retry was broken — the retry path set step status to `"PENDING"` but the worker loop only restarts `"QUEUED"` steps. Steps scheduled for retry were permanently stuck with no second execution. Changed retry status to `"QUEUED"` so the outer loop picks them up correctly.
...
- RPS version number logged at startup immediately after the "Log file:" line (`rps.logging` logger). Reads the installed package version via `importlib.metadata` so the container version is visible in every runtime log without grepping binaries.

## [0.25.0] - 2026-09-16

### Fixed

- Fixed QUEUED runs permanently blocking new runs when their queue item was moved to `failed/` before the worker could start (e.g. after an empty-file read error). New `_recover_orphaned_queued_runs` in `queue_scheduler.py` detects QUEUED/RUNNING runs with no `pending/` or `active/` queue item and fails them at startup and every 60 s in the loop. 3 new tests in `tests/test_run_scheduler_resilience.py`.
...
- Added bounded day-swap operation to the Workout Editor (`FEAT_workout_editor_swap_days`). A new `preview_swap_workouts()` function in `week_plan_edits.py` swaps the workouts (and their agenda metadata: day_role, planned_duration, planned_kj) between two occupied days in the same week plan, updating each workout's date to its new day. If either day lacks a workout, a clear error is returned directing the user to `preview_move_workout` instead. The Coach can invoke it as the new `preview_swap_workouts` tool (added to the preview toolset). 5 unit tests in `tests/test_workout_editor_swap_days.py`.

- Added visual run progress indicators to the Plan Hub (`FEAT_run_progress_ui`). While a planning run is active, the "Run Execution" section now shows a `st.progress()` bar displaying pipeline step completion (N / M steps) alongside elapsed time since the run started. The current step label and CrewAI runtime detail (flow, crew, task, agent) are consolidated into a single `st.info()` line for immediate visibility, replacing the scattered small captions. After a run completes, the existing caption-based display is restored. The auto-refresh (every 2 s) is unchanged.

- Completed `plan_week` testability and resolution refactor (`FEAT_plan_week_testability_and_resolution_refactor`). All four PR slices complete: (PR1) shared test helpers extracted into `tests/planning_context_helpers.py`; (PR2) typed orchestration dataclasses (`PlanWeekResult`, `PlanWeekRequest`, `PreviousWeekReportGateOutcome`, `PhaseRefreshPlan`, etc.) introduced in `plan_week.py`; (PR3) `_run_phase_bundle()` and `_run_week_planning()` extracted from the two ~260-line inline blocks, reducing `plan_week()` to ~300 lines; (PR4) `PhaseExecutionResolution` dataclass and `_resolve_phase_execution_roles()` introduced in `deterministic_context.py`, with focused unit tests for all five Phase-1 cadence/role fallback scenarios (active slot, phase_raw fallback, pattern-derived fallback, unsupported cadence blocking issue, role-count mismatch blocking issue).
...
- Improved Docker build and deployment workflow (`FEAT_docker_deploy`). Added `plotly>=5.22.0` to `pyproject.toml` runtime dependencies (it was present in `requirements.txt` but missing from `pyproject.toml`); removed the redundant `altair>=5.0.1` entry (transitive via Streamlit, no direct import). Updated `.dockerignore` to exclude `.venv/`, `tests/`, `doc/`, and `memory/` from the image build context. Removed the deprecated `version: "3.9"` key from `docker-compose.yml` and added a commented config-volume block so operators can enable opt-in auth (`RPS_USERS_FILE`) without changing the compose file structure. Added GitHub Actions BuildKit layer cache (`type=gha`) to the GHCR build workflow (`.github/workflows/ghcr-image.yml`) — pip-install layers now cache across pushes when `requirements.txt` is unchanged. Added `scripts/docker_build.sh` as a local build/push helper (`TAG=x.y.z ./scripts/docker_build.sh --push`).

- Added opt-in authentication and per-user athlete ID (`FEAT_user_management`). A new module `src/rps/ui/auth.py` provides `hash_password()` (SHA-256, `sha256:<hex>` format), `verify_password()`, `load_user_registry()` (reads a YAML file at `RPS_USERS_FILE`), and `require_auth()` (shows a login form and calls `st.stop()` when the user is not authenticated). When `RPS_USERS_FILE` is not set, all auth functions are no-ops and the app behaves exactly as before. After login, `get_athlete_id()` / `init_ui_state()` return the `athlete_id` from the authenticated user record; the sidebar shows the logged-in user and a Logout button instead of the athlete_id text input. Optional `llm_api_key` per user overrides `RPS_LLM_API_KEY` in `os.environ` at login time (single-host caveat documented). CLI helper: `python -m rps.ui.auth hash <password>`. Sample registry at `config/users.yaml.example`. 15 new tests in `tests/test_auth.py`.

- Added input-driven plan adjustment suggestions (`FEAT_plan_adjustments`). A new module `src/rps/orchestrator/plan_adjustments.py` provides `get_adjustment_suggestions()` which compares each modular input's `created_at` timestamp against the downstream plan artifact it influences (ATHLETE_PROFILE → Season Plan, PLANNING_EVENTS → Season Plan, AVAILABILITY → Phase, LOGISTICS → Week Plan). When an input is newer than its plan artifact (and both exist), an `AdjustmentSuggestion` is emitted. Duplicate scopes are deduplicated — the most-recently-updated input wins. Results are sorted by scope priority (Season Plan > Phase > Week Plan). The Plan Hub (`src/rps/ui/pages/plan/hub.py`) gains a "Plan Adjustment" section rendered above the Readiness panel whenever suggestions exist; each row shows which input changed, when, and a "Run [Scope] Adjustment" button that queues the appropriate scoped re-run through the existing scoped-run machinery. 21 new tests in `tests/test_plan_adjustments.py`.

- Added per-input-type JSON export/import (`FEAT_user_inputs_io`). A new shared helper `src/rps/workspace/input_io.py` provides `export_input()`, `import_input()`, and `suggest_export_filename()` for the four modular input artifact types (ATHLETE_PROFILE, AVAILABILITY, PLANNING_EVENTS, LOGISTICS). Each input editor page (About You & Goals, Availability, Events, Logistics) gains a collapsed "Export / Import" expander with a "Download as JSON" button and a file uploader; imports validate the artifact-type header and write a new version via the existing `save_version` path. Type-mismatch and parse errors are shown in the UI without writing anything. 14 new tests in `tests/test_input_io.py`.

- Added stuck-run detection and recovery (`FEAT_run_scheduler_resilience`). Three recovery mechanisms: (1) `_lock_is_stale()` + `_recover_stale_lock()` in `run_store.py` detect and clear lock files whose run is already terminal, missing, or older than 4 hours — `acquire_athlete_lock()` now retries once after a successful stale-lock recovery so callers no longer need to handle the "Athlete lock busy" case from a crashed process; (2) `_recover_orphaned_active_items()` in `queue_scheduler.py` resolves items stranded in `queue/active/` on scheduler startup — fails their RUNNING/QUEUED runs and moves the items to `failed/`; (3) `_recover_stuck_runs()` sweeps all per-athlete lock files on startup and clears any stale ones. Both startup helpers are called before the scheduler's poll loop begins and are non-fatal (log a warning on error). All recovery emits a `RUN_FAILED` event with a descriptive reason so History/audit is preserved. 26 new tests in `tests/test_run_scheduler_resilience.py`.

- Extracted `_run_phase_bundle()` and `_run_week_planning()` (`src/rps/orchestrator/plan_week.py`) from the two ~260-line inline orchestration blocks in `plan_week()`. Each returns a typed outcome dataclass (`PhaseBundleOutcome`, `WeekPlanningOutcome`) carrying the new steps, an error flag, and state the caller needs for the export gate — the caller simply extends `steps` and checks the error/isolated-run flags. The inline `_mtime` nested function was promoted to module level with a typed `Path | None` signature. `plan_week()` is now ~300 lines (down from ~900). Part of `FEAT_plan_week_testability_and_resolution_refactor` (PR3 slice).
- Extracted `_resolve_phase_refresh_plan()` (`src/rps/orchestrator/plan_week.py`) from the inline phase-refresh-plan logic in `plan_week()`, replacing the 30-line inline block with a pure function that takes explicit boolean/mtime inputs and returns a typed `PhaseRefreshPlan`. Added six focused unit tests in `tests/test_plan_week_helpers.py` covering all-missing, all-present, stale-guardrails cascade, isolated forced-guardrails, forced-structure-includes-preview, and return-type cases. Added `seed_plan_hub_readiness_inputs()` helper to `tests/planning_context_helpers.py` (wraps the repeated ATHLETE_PROFILE/PLANNING_EVENTS/LOGISTICS/AVAILABILITY/KPI_PROFILE/WELLNESS/scenario-chain setup used by plan-hub readiness tests) and switched `test_plan_hub_readiness_requires_latest_files` and `test_plan_hub_marks_selection_stale_after_new_scenarios` (`tests/test_plan_hub_page.py`) to use it, reducing per-test inline setup from ~25 to ~2 lines each. Part of `FEAT_plan_week_testability_and_resolution_refactor` (PR3 slice).

- Added `build_resolved_report_evidence_block` (`src/rps/orchestrator/resolved_context.py`) and `_build_prior_phase_artefacts_block` (`src/rps/orchestrator/plan_week.py`) to close the two remaining content-injection gaps for `phase_planning`: previous-week `DES_ANALYSIS_REPORT` content (kpi_summary status/confidence, weekly interpretation summary, trend observations, recommendation urgency/rationale) and, on a re-plan, any already-stored exact-range `PHASE_GUARDRAILS`/`PHASE_STRUCTURE`/`PHASE_PREVIEW` artefacts. Both are now injected as text at the phase-architect call site alongside the existing `build_resolved_activity_context_block` call, matching `season_planning`'s pattern exactly.
- Injected the whole-phase deterministic execution context (`build_phase_execution_context`/`render_phase_execution_context_block`, already used at the phase-architect call site) into `week_planning`'s `user_input` (`plan_week.py`'s week-planner call site). This closed the one real content gap found for week planning: `week_calendar_block` and `planning_context_snapshot_block` already carried the target week's own slice of phase authority, but not whole-phase fields like cadence family, every week's role/load-band, or deload intent/rationale — the same fields `workspace_get_phase_execution_context` used to fetch. `phase_slot_context` is passed as `{}` here since the phase is already persisted in `season_plan` by the time weeks are planned; `build_phase_execution_context` falls back to the phase's own persisted cadence/week-role data the same way.
- Injected the completed-week activity evidence, trend context, KPI bands, and phase context into the DES report's `user_input` (`plan_week.py`'s `create_performance_report`) via `build_resolved_activity_context_block`/`build_resolved_kpi_context_block`/`build_resolved_phase_context_block`. The `des_diagnostic_draft` task has no workspace tools, but the prompt only injected version-key strings (the deterministic report-evidence block) and told the model to "use `workspace_get_version`" / "read KPI profile, season plan, and phase context from workspace" — instructions for tools the task cannot call, leaving the diagnostic to run on far less concrete evidence than its `analysis-methodology` skill expects.
...
- Fixed a duplicate-content bug introduced earlier in this slice: the phase-architect call site's `injected_block` called `build_resolved_activity_context_block` directly even though `planning_context_snapshot_block` (via `save_planning_context_snapshot`, called with the same versions) already rendered the identical "**Resolved Activity Context**" block. Found by executing `build_planning_context_snapshot_document` directly while verifying the week-planning analysis, rather than only reading the code. Removed the redundant call; added a regression test asserting the block appears exactly once in the captured phase `user_input`.
- Removed two stale `workspace_get_version` tool-usage instructions at the week-planner call site (`plan_week.py`) that told the model to fetch content already injected as actual data, not just version keys — the same bug class already found and fixed in `season_flow.py`'s `historical_context_line` this session: `historical_context_line` (now emptied for week, matching phase) and the opening "Read phase_guardrails and phase_structure from workspace... use `workspace_get_version`..." sentence in the week `user_input` (reworded to state the data is already injected). Deleted `_build_historical_context_line` entirely once it became unused at both remaining call sites.
- Removed the three stale tool-usage sentences from the DES report `user_input` ("Use `workspace_get_version` for target-week activity artefacts...", "Load ACTIVITIES_ACTUAL version_key X...", "Read KPI profile, season plan, and phase context from workspace") now that the content is injected as text (see Added).

### Removed

- Extended the `season_planning` tool-redundancy fix to `phase_planning` (9 of 10 tool-equipped tasks), `season_review` (all 5 tasks), and `phase_review` (all 6 tasks) — 21 tasks total, `config/crewai/tasks.yaml`. Traced `_run_multicrew_cycle`/`_run_review_decision_document` (`src/rps/agents/crewai_task_execution.py`) and confirmed review crews receive the exact same injected `user_input` their planning crew received, plus the candidate bundle on top — no content gap exists for review tasks at all, so the same redundancy argument applies without qualification. For `phase_planning`, closed the two remaining gaps (see Added) first so the same "tools are provably redundant, not just softened to fallback" argument held for every task, not just most of them. Removed the now-dead dynamic tool-override machinery this made redundant (`_phase_bundle_finalize_has_bound_contracts` in `crewai_context_blocks.py`, the `tools_override_by_task` parameter threaded through `_run_phase_bundle_document`/`_execute_crewai_multiagent_crew`) since `phase_bundle_finalize`'s tools are now statically empty by construction. Reworded 12 skill/prompt files (`prompts/agents/phase_bundle_manager.md`, `phase_architect.md`, `season_planner.md`; `skills/phase/{bundle-synthesis,constraint-audit,review-decision,context-analysis,structure-review}/SKILL.md`; `skills/season/{plan-synthesis,constraint-synthesis,context-analysis,review-decision}/SKILL.md`) that still described tool-usage patterns for the now-removed tools.
- Extended the same tool-redundancy fix to `week_planning` (6 of 7 tool-equipped tasks) and `week_review` (all 5 tasks) — 11 tasks total, `config/crewai/tasks.yaml`. Unlike Season/Season-review/Phase-review, week planning had one genuine content gap rather than zero (see Added); an initial scoped investigation concluded otherwise, but directly executing `build_planning_context_snapshot_document` (rather than only reading it) showed `planning_context_snapshot_block` already injects activity/report/recovery/load-governance/phase-authority content for week planning via the same snapshot mechanism used for phase planning, since the week call site additionally passes `phase_guardrails_payload`/`phase_structure_payload`. Reworded 6 skill files (`skills/week/{workout-construction,constraint-analysis,plan-synthesis,review-decision,context-analysis,revision-methodology}/SKILL.md`) that still described tool-usage patterns for the now-removed tools.
- Deleted the dead `report_context_read` task and its `performance_context_specialist` agent. The task was never executed — the DES report flow (`_run_single_internal_document`) runs single tasks and ignores `context:` chains, so `report_context_read` existed only as an unused entry in `report_review`'s `context:` list — yet it carried a full config footprint (`tools:`, agent definition, and entries in `agents.yaml`/`skills.yaml`/`memory_policy.yaml`/`runtime_profiles.yaml`/`knowledge_sources.yaml`) plus the `skills/report/context-analysis/` skill, all now removed.

## [0.20.30] - 2026-07-14

### Added

- Added a systematic root-cause investigation of the near-universal `typed_output_present` first-attempt guardrail failures (16 of 18 tasks failed their first attempt across `season_planning`/`season_review` in the latest run, roughly doubling LLM calls). Traced `events.jsonl` directly: 13 of 16 failing raw outputs are complete, syntactically valid JSON that validates cleanly against their target pydantic model when checked directly (`SeasonEventAnchorModel.model_validate(json.loads(raw))` succeeds with zero errors on the exact text CrewAI's guardrail reported as "no typed structured output") — ruling out content, prompt, and schema problems. Found a strong correlation with tool usage in `season_planning` (10 of 11 tool-equipped tasks failed; both non-tool tasks succeeded) and, via CrewAI GitHub/community research, a documented bug class combining `output_pydantic` + custom `guardrail` + task-`context` chaining ([crewAIInc/crewAI#5458](https://github.com/crewAIInc/crewAI/issues/5458)) that matches this architecture closely. `season_review`'s 5/5 first-attempt failure rate despite already-conservative tool phrasing broadens the likely dominant trigger to CrewAI's own `context:` task-chaining conversion path rather than tool usage alone — every multi-task crew here uses `context:` chaining. This is very likely a CrewAI-side limitation rather than something fixable in our own guardrail code; the plan is documented at a fixed point in time for reference, not as a permanent design doc.

### Changed

- Raised the guardrail-failure diagnostic text cap from 8000 to 50000 chars (new shared `DIAGNOSTIC_TEXT_LIMIT` constant in `src/rps/crewai_runtime/guardrails_utilities.py`, used by `_raw_output_snippet`, the `reason`/`prompt_snippet` fields in `_with_guardrail_telemetry`, and `typed_output_present`'s inline raw-output detail in `guardrails_generic.py`). A live run showed 3 of 16 guardrail-failure raw outputs still hitting the previous 8000-char cap mid-JSON, leaving open whether the underlying LLM completion itself was truncated by `max_completion_tokens` (a real, separate bug) or just our own capture — this closes that diagnostic gap for the next run.
- Added an explicit "never wrap your final structured output in markdown code fences" rule to `_INTERNAL_TOOL_FIRST_RULES` (`src/rps/agents/crewai_task_execution.py`), the shared prompt scaffold for every internal CrewAI task. Markdown code-fencing (` ```json ... ``` `) is a documented, independent cause of `TaskOutput.pydantic` ending up `None` in CrewAI community reports; cheap to guard against regardless of whether it's the dominant cause of the failures investigated above.

### Fixed

### Removed

- Removed all workspace tools (and their "Tool usage:" description text) from all 11 tool-equipped `season_planning` tasks (`config/crewai/tasks.yaml`: `season_context_read`, `season_event_priority_review`, `season_peak_window_review`, `season_macrocycle_draft`, `season_constraint_review`, `season_historical_context_review`, `season_kpi_guidance_review`, `season_load_corridor_draft`, `season_progression_review`, `season_phase_blueprint_draft`, `season_plan_finalize`). Traced `src/rps/orchestrator/season_flow.py`'s `create_season_plan` and confirmed every one of these tools' targets (athlete profile, availability, logistics, planning events, KPI profile, season scenarios/selection, previous-week activities, historical baseline, deterministic phase-slot/load context) is already deterministically resolved and injected as text into the crew's shared `user_input` before any CrewAI task runs. An initial pass only softened the tool-usage instructions to "fallback only" phrasing and kept the tools configured, reasoning they were a defensive safety net — on reflection that reasoning didn't hold: `workspace_get_input`/`workspace_get_latest` read from the exact same `LocalArtifactStore` "latest" lookup the injection already used, so the tool can never succeed where the injection didn't (the only way it could is a bug in the injection code itself, which is a testing problem, not something an LLM escape-hatch should paper over); the two "contract" tools read the identical `current_guardrail_runtime_context()` ContextVar the injected phase-slot/phase-load text blocks are rendered from. Removed outright instead, since keeping a tool the model might call "just to be safe" is exactly the exposure to CrewAI's less-reliable tool-calling/`AgentExecutor` conversion path this change is meant to reduce. Updated 4 tests across `test_crewai_config_and_builders.py`, `test_crewai_phase_week_review_guardrails.py`, and `test_crewai_season_semantics_normalization.py` that asserted the old tool lists. Scope limited to `season_planning`; `phase_planning`/`week_planning`/`season_review` crews use separate context-injection paths not yet audited the same way — flagged as a follow-up, not assumed to have the same redundancy.
...
- Bumped guardrail-failure diagnostic truncation limits by roughly 15-20x (`typed_output_present`'s inline raw-output text 300 → 6000 chars, `_raw_output_snippet` 400 → 8000, the shared `reason`/`prompt_snippet` cap in `_with_guardrail_telemetry` 500 → 8000) after a real run showed every visible raw-output snippet already looking like well-formed JSON matching the expected schema's first fields — meaning the actual pydantic validation failure is on a field past the old, too-short truncation window, not diagnosable from what was captured.
- Fixed `workspace_get_latest`/`workspace_get_version`/`workspace_list_versions`/`workspace_find_best_phase_artefact` (`src/rps/tools/workspace_read_tools.py`) raising a bare `KeyError: 'artifact_type'` (and `workspace_get_version` a bare `KeyError: 'version_key'`) when an LLM tool call omits the field from `payload_json`. A real production run showed this exact failure: `season_constraint_review` called `workspace_get_latest` without `artifact_type`, the tool-call wrapper's `except Exception: str(exc)` turned the KeyError into the bare string `'artifact_type'` as the tool's `error` field, the LLM had no information to self-correct, and reported the failure as a blocking issue — which then correctly (but misleadingly) blocked the whole season plan write via `season_bundle_review_readiness`, surfacing as `SEASON_BUNDLE_RAW_VALIDATION_FAILED` with a reason that looked like a genuine content-level planning conflict but was actually this tool-call bug. New `_require_artifact_type` helper raises a clear, example-bearing `ValueError` instead, and `workspace_get_version` now validates `version_key` the same way.
- Added `raw_output_snippet` and `prompt_snippet` diagnostic fields to every `CREW_TASK_GUARDRAIL_FAILED` event (`src/rps/crewai_runtime/guardrails_utilities.py`'s `_with_guardrail_telemetry`, generalizing what was previously only appended to `typed_output_present`'s own reason string). `raw_output_snippet` captures the raw/JSON text CrewAI's LLM call actually returned (truncated); `prompt_snippet` captures the task description that was actually sent, threaded through from the 3 production `Task(...)` construction sites that have it (`_build_crewai_task`, `_execute_crewai_task` in `src/rps/agents/crewai_task_execution.py`, and `bindings.py`'s task builder) via a new optional `task_description` parameter on `build_task_guardrail_kwargs`. Both fields are diagnostic-only (not surfaced in the compact `rps.log` line, which only extracts a fixed key set) and land in the full JSON event (`runs/<run_id>/events.jsonl`), so a future guardrail failure is directly diagnosable from what was actually asked and returned, without needing another live run and pasted log to investigate — closing the exact gap that made diagnosing the `typed_output_present`/nano-model investigation earlier in this session require multiple live runs.
- Added a "final answer discipline" rule to `_INTERNAL_TOOL_FIRST_RULES` (`src/rps/agents/crewai_task_execution.py`), the shared prompt scaffold for practically every internal CrewAI task across Season/Phase/Week (including season_planning and season_review): "once you have gathered what you need, your final response must be the structured output only — no narration, no tool-call summary, no prose before or after." A live run after the two `typed_output_present` fixes showed a suspiciously uniform pattern — nearly every one of 18 tasks across different models (nano/mini/full) failed its *first* guardrail attempt and succeeded on retry, too consistent to be organic model unreliability. The shared task-description builder had extensive tool-usage rules but no explicit instruction that the final response, once tools are done, must be pure structured output — a known cause of exactly this "trails into prose after the last tool call" pattern. Also added raw-output-snippet diagnostics to `typed_output_present`'s failure reason (`src/rps/crewai_runtime/guardrails_generic.py`) so a future occurrence is directly diagnosable from `CREW_TASK_GUARDRAIL_FAILED` telemetry instead of requiring another live run to investigate.
- Added `doc/specs/features/FEAT_finalize_manager_synthesis_boundary.md`, `SeasonPlanManagerSynthesisModel`, `PhaseBundleManagerSynthesisModel`, and `WeekPlanningNoteModel`. Redesigned `season_plan_finalize` and `phase_bundle_finalize` around a "manager synthesis vs. deterministic assembly" pattern: a production incident showed `season_plan_finalize` failing after 3 wasted LLM calls because its task description asked the LLM to reproduce `constraints[]`/`load_governance[]`/`phase_blueprints`, fields already produced in an already-typed shape by same-crew sibling tasks. Both finalize tasks now emit a narrower synthesis excluding any field 100% derivable from an upstream sibling; `_assemble_season_plan_draft_bundle`/`_assemble_phase_draft_bundle` (`src/rps/agents/crewai_output_extraction.py`) merge the narrow synthesis with the siblings' already-typed outputs into the full, unchanged `SeasonPlanDraftBundleModel`/`PhaseDraftBundleModel` contract immediately after `crew.kickoff()`. `season_plan_finalize` reverts to `output_mode: pydantic` (previously moved to `json` by `FEAT_season_finalize_raw_bundle_boundary`, now superseded, to work around strict-binding rejections that this redesign eliminates at the source instead of coercing around). Guardrails that depended on the removed pass-through fields (`season_bundle_integrity`, `season_bundle_audit_slot_integrity`, `season_phase_load_feasibility`, `season_bundle_review_readiness`, `phase_bundle_integrity`, `phase_bundle_review_readiness`) moved from CrewAI's guardrail/retry chain to plain post-assembly Python checks in `_run_season_plan_document`/`_run_phase_bundle_document` (still fail-closed, just invoked directly since retrying the LLM cannot fix a code-assembly issue). Verified the same reproduction pattern does not exist in Week's finalizer (its upstream draft tasks produce untyped narrative with no type match to the day/workout blueprints `week_plan_finalize` produces); Week's only related gap — `WeekPlanBundleModel.constraint_summary`/`load_target_summary`/`revision_summary` being untyped `list[str]` — is fixed with the new `WeekPlanningNoteModel`. Updated prompt/skill content wherever it instructed reproduction of a now-removed field (`prompts/agents/season_plan_manager.md`, `phase_bundle_manager.md`, `season_phase_blueprint_specialist.md`, `guardrails_specialist.md`, `structure_specialist.md`; `skills/season/plan-synthesis`, `skills/phase/bundle-synthesis`, `guardrails-authoring`, `structure-authoring`), including moving the `inherited_scenario_contract` "freeze exactly" discipline from the Phase manager (which no longer sees the field) to `guardrails_specialist`/`structure_specialist` (the actual point of authorship now that there is no reproduction step to re-freeze it at).
- Added `doc/adr/ADR-061-crewai-guardrails-module-split.md` and `doc/specs/features/FEAT_guardrails_module_split.md`, planning a 7-phase staged split of `src/rps/crewai_runtime/guardrails.py` (2346 lines) into focused modules (context core, generic/schema validators, cross-domain utilities, phase/week/season domain validators, registry). Confirms neither the `ContextVar`-based runtime-context mechanism nor the `REGISTRY` string-resolution pattern blocks a same-behavior split, mirroring ADR-060's finding for `crewai_backend.py`.
...
- Removed `src/rps/tools/workspace_tools.py` (492 lines), a near-duplicate of `workspace_read_tools.py` with zero production consumers (confirmed via `grep -rln`) and only one test referencing it, which duplicated coverage already present for `workspace_read_tools.py`'s equivalent. It still carried the unpatched `_parse_artifact_type(args["artifact_type"])`-style bare-subscript bugs already fixed in its sibling — a maintenance trap for anyone reading or reactivating it. Found during a systematic audit of workspace tool cleanliness/shift-left opportunities.

### Changed

- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 1 of a staged module split, no ADR required — pure internal reorganization, not touching persistence strategy/orchestration/authority/telemetry boundaries): moved ISO-week/date utilities and CLI argument parsing into a new `src/rps/data_pipeline/intervals_date_utils.py`, and the Intervals.icu HTTP API client (session/retry policy, raw fetch helpers) into a new `src/rps/data_pipeline/intervals_api_client.py`. `intervals_data.py` imports both back for its own use. No behavior change; `tests/test_intervals_data.py` needed no changes (it only tests `_power_zone_share_percent`, which didn't move).
- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 2): moved athlete-profile parsing and zone-model construction into `src/rps/data_pipeline/intervals_zone_model.py`, wellness fetching/writing into `src/rps/data_pipeline/intervals_wellness.py`, and historical-baseline aggregation into `src/rps/data_pipeline/intervals_historical_baseline.py`. Also extracted the shared schema/validation utilities (`_canonicalize_pipeline_payload`, `_value_present`, `_confidence_from_columns` — used by every output-writing stage) into `src/rps/data_pipeline/intervals_schema_utils.py`, earlier than originally planned, once the dependency audit showed all three Phase-2 modules needed it and importing it back from `intervals_data.py` would have created a circular import. Also found and resolved a real cross-cluster dependency the initial structural map missed: wellness writing calls 4 of zone-model's athlete-profile parsing helpers (`_extract_weight_kg`, `_extract_latest_weight_from_wellness`, `_parse_date`, `_num_or_none`) plus a shared `normalize_scalar`/3 tiny constants, now a one-way import from `intervals_wellness.py` into `intervals_zone_model.py`. No behavior change.
- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 3): moved column formatting/unit-conversion/rounding-policy helpers into `src/rps/data_pipeline/intervals_formatting.py`, and JSON-safe scalar normalization/formatting helpers into `src/rps/data_pipeline/intervals_json_formatters.py`. Found another cross-cluster dependency: `format_duration_hms` (JSON formatters) calls `seconds_to_hms` (formatting), now a one-way import. No behavior change; `FLAG_COLUMN_KEYS`, `IndexMeta`, and `_power_zone_share_percent` — interleaved between the two moved groups' boundaries but belonging to neither — were caught and restored to the residual after an overly broad line-range deletion briefly removed them, verified via a fresh `py_compile`/`ruff` pass before finalizing.
- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 4): moved export DataFrame building (`build_export_dataframe`, `export_range`, plus internal `to_num`/`div_or_none` helpers) into `src/rps/data_pipeline/intervals_export.py`. Clean extraction, no new cross-cluster dependencies. No behavior change.
- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 5): moved activities-actual schema transformation (`build_activities_actual_payloads_from_export_frame`, `fetch_current_week_activities_actual_payload`, `compile_activities_actual`) into `src/rps/data_pipeline/intervals_activities_actual.py`. This is the first phase of the split with an external consumer beyond `intervals_data.py` itself: `src/rps/orchestrator/context_snapshots.py` imported `fetch_current_week_activities_actual_payload` directly, so its import path was repointed to the new module. `intervals_data.py`'s residual keeps only its internal call to `compile_activities_actual`. No behavior change; `intervals_data.py` down to 951 lines (from 2890 originally).
- Changed `src/rps/data_pipeline/intervals_data.py` (Phase 6, final data-pipeline extraction): moved activities-trend weekly aggregation (`compile_activities_trend`, its 13 nested helper closures, `load_kj_plan_by_week`, `_power_zone_share_percent`, `FLAG_COLUMN_KEYS`, `IndexMeta`, and the export-configuration constants block) into `src/rps/data_pipeline/intervals_activities_trend.py`, once the audit confirmed all of it was exclusively used by `compile_activities_trend`. Also deleted two genuinely dead leftover constants (`PERCENT_SCALE_THRESHOLD`, `PERCENT_INTEGER_EPSILON`) that were already duplicated in `intervals_zone_model.py` since Phase 2 and had no remaining call sites in `intervals_data.py`. `tests/test_intervals_data.py`'s import of `_power_zone_share_percent` repointed to the new module. `intervals_data.py`'s residual is now purely the orchestration entrypoint (`parse_args`/`run_pipeline`/`main`), 139 lines (from 2890 originally) — this phase completes the `intervals_data.py` module split.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 1 of the ADR-061 staged module split): moved the `ContextVar`-based runtime-context threading mechanism (`_GUARDRAIL_CONTEXT`, `guardrail_runtime_context`, `current_guardrail_runtime_context`) and the shared `JsonMap`/`GuardrailResult`/`GuardrailFn` type aliases into a new `src/rps/crewai_runtime/guardrails_context.py`. `guardrails.py` imports all of these back for its own internal use (many validators call `current_guardrail_runtime_context()` or reference `_GUARDRAIL_CONTEXT` directly). Updated the 7 production files and 11 test files that imported `guardrail_runtime_context`/`current_guardrail_runtime_context` directly from `guardrails.py` to import from the new module instead — no re-export shim left behind, per this repo's established convention. No behavior change.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 2 of the ADR-061 staged module split): moved the 6 generic output-shape validators into a new `src/rps/crewai_runtime/guardrails_generic.py`, and the 3 artifact-envelope/JSON-Schema validators (`artifact_envelope_basic`, `artifact_meta_data_present`, `artifact_schema_valid`, plus the `_schema_registry()` cache helper) into a new `src/rps/crewai_runtime/guardrails_schema.py`. Discovered a circular-import risk the ADR's audit didn't anticipate: both new modules need the payload-coercion helpers `_coerce_payload`/`_coerce_mapping`, which were still defined in `guardrails.py` at the time (scheduled for Phase 3) — importing them from there would create a cycle once `guardrails.py`'s `REGISTRY` imports the moved validators back. Resolved the same way `intervals_schema_utils.py` was pulled forward in the `intervals_data.py` split: extracted just those 2 helpers early into a new `src/rps/crewai_runtime/guardrails_utilities.py` (the rest of Phase 3's utilities group still to come). Also deleted `ROOT`/`SCHEMA_DIR`, which became dead in `guardrails.py`'s residual once their only consumer (`_schema_registry()`) moved — a duplicate copy already exists in `guardrails_schema.py`. No behavior change.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 3 of the ADR-061 staged module split, completes the cross-domain utilities group): moved the remaining 22 utility functions/constants (`_as_map`/`_as_list`/`_as_float`/`_string_list`, diagnostics helpers, `_with_guardrail_telemetry`, `canonicalize_season_bundle_shape_aliases`, `decode_json_object_from_text`, `normalize_artifact_candidate_for_task_guardrails`, context accessors, etc.) into `src/rps/crewai_runtime/guardrails_utilities.py`, alongside the payload-coercion helpers pulled forward in Phase 2. Used an AST-based extraction script (locating each name's exact source range via `ast.parse`) rather than manual line-range `sed`, since these 22 names are physically interleaved throughout the file rather than contiguous — the approach that caused self-caught boundary mistakes in the `intervals_data.py` split. Found one real external consumer the ADR's audit missed: `src/rps/agents/crewai_output_extraction.py` imported `canonicalize_season_bundle_shape_aliases`/`decode_json_object_from_text` directly from `guardrails.py`, repointed to the new module. Also found and fixed a subtler break: two test files (`tests/test_crewai_config_and_builders.py`, `tests/test_crewai_phase_writer_guardrails.py`) monkeypatched `crewai_guardrails.emit_runtime_event` / `crewai_guardrails.normalize_artifact_candidate_for_task_guardrails` — since `_with_guardrail_telemetry` now lives in `guardrails_utilities.py` and resolves those names in its own module's namespace, patching the attribute on the old `guardrails` module silently stopped intercepting the call; repointed both monkeypatches (and 6 direct-call sites across 2 test files) to `guardrails_utilities`. No behavior change.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 4 of the ADR-061 staged module split): moved the 7 phase-artifact validators (`phase_bundle_integrity`, `phase_bundle_matches_context`, `phase_bundle_review_readiness`, `phase_execution_context_match`, `phase_weeks_match_range`, `phase_week_role_load_coherence`, `phase_s5_band_match`) plus their exclusive ISO-week helpers (`_extract_expected_s5_band`, `_check_role_band_sequence`, `_iso_week_key`, `_weeks_in_range`, `_coerce_week_key`) into a new `src/rps/crewai_runtime/guardrails_phase.py`. Found a cross-cluster dependency the ADR's audit didn't separate out: `_next_iso_week`, physically adjacent to the other ISO-week helpers, is exclusively called by `season_phase_coverage_and_cadence` (a season-group function not yet extracted) and not by any phase-group function — left in `guardrails.py`'s residual to move with the season group in Phase 6 instead of following the other 4 helpers. Updated `src/rps/agents/crewai_validation.py` (1 of the original 8 production consumers) and 3 test files whose imports mixed moving and staying names. No behavior change.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 5 of the ADR-061 staged module split): moved the 14 week-artifact validators (`week_bundle_integrity`, `week_bundle_matches_context`, `week_bundle_domain_legality_check`, `week_bundle_review_readiness`, `review_decision_integrity`, `week_corridor_and_capacity_check`, `week_active_corridor_match`, `week_recovery_day_load_check`, `week_agenda_shape_and_calendar_check`, `week_daily_availability_check`, `week_phase_role_alignment_check`, `week_contract_context_match`, `week_exportability_check`, `week_workout_structure_policy_check`) plus `week_bundle_domain_legality_messages`, `_normalized_domain_token`, the workout-domain-analysis helpers, `_target_week_from_context_or_meta`, and `des_diagnostic_only` into a new `src/rps/crewai_runtime/guardrails_week.py`. Found another cross-cluster dependency: `_repair_season_plan_for_contract_validation`, physically located within this cluster's line range, is exclusively called by `season_phase_load_context_match` (a season-group function) — left behind for Phase 6, same resolution as Phase 4's `_next_iso_week` finding. Updated `src/rps/agents/crewai_task_execution.py` and `src/rps/agents/crewai_validation.py` (2 of the original 8 production consumers) and 5 test files. No behavior change.
- Changed `src/rps/crewai_runtime/guardrails.py` (Phase 6 of the ADR-061 staged module split, the largest and highest-risk phase): moved the 12 season-artifact validators (`season_bundle_integrity`, `season_bundle_audit_slot_integrity`, `season_bundle_matches_contract`, `season_bundle_review_readiness`, `season_writer_bundle_match`, `season_phase_load_feasibility`, `season_scenario_selection_shape`, `season_scenarios_profile_quality` — 440 lines alone — `season_scenarios_selection_contract_complete`, `season_phase_coverage_and_cadence`, `season_phase_load_context_match`, `season_cycle_ordering`) plus the 11 marker-tuple constants and the two cross-cluster helpers left behind from Phases 4-5 (`_next_iso_week`, `_repair_season_plan_for_contract_validation`) into a new `src/rps/crewai_runtime/guardrails_season.py`. `guardrails.py`'s residual is now purely `TaskExecutionPolicy`/`REGISTRY`/the 3 resolution functions (176 lines, from 2346 originally). Updated `src/rps/agents/crewai_validation.py` (1 of the original 8 production consumers) and 6 test files. Full suite (623/623) confirmed the season group's behavior, including `season_scenarios_profile_quality`, is unchanged. No behavior change.
- Removed `src/rps/crewai_runtime/guardrails.py` (Phase 7 of the ADR-061 staged module split, final phase, completes the split). Its remaining content (`TaskExecutionPolicy`, `REGISTRY`, `resolve_guardrail`, `resolve_task_policy`, `build_task_guardrail_kwargs`) moved verbatim into a new `src/rps/crewai_runtime/guardrails_registry.py`, matching `crewai_backend.py`'s full retirement after ADR-059/060. Found one relative-import consumer (`src/rps/crewai_runtime/bindings.py`'s `from .guardrails import ...`) an absolute-path grep had missed; repointed to `.guardrails_registry`. Updated 5 test files. Verified every guardrail name string in `config/crewai/task_policies.yaml` (41 configured names) resolves against the new `REGISTRY` (43 entries) via a script iterating `resolve_guardrail()` for each — confirming the string-based config-resolution contract is unchanged after moving the Python-side lookup module. `src/rps/crewai_runtime/guardrails.py`'s original 2346 lines now live across 8 focused modules (`guardrails_context.py`, `guardrails_generic.py`, `guardrails_schema.py`, `guardrails_utilities.py`, `guardrails_phase.py`, `guardrails_week.py`, `guardrails_season.py`, `guardrails_registry.py`), completing the ADR-061 split across all 7 phases. No behavior change.
...
- Changed `src/rps/planning/deterministic_context.py` so `build_load_capacity_block(...)` now routes through a typed `LoadCapacityContext` wrapper with explicit `to_payload()` projection, preserving the existing dict-compatible load-capacity payload contract while introducing an internal typed seam for the next migration slices.
...
- Changed `src/rps/agents/crewai_backend.py` (Phase 2 of the staged module split, see ADR-059): moved the 7 bundle/artifact-validation and meta-normalization functions into a new `src/rps/agents/crewai_validation.py` module, with `crewai_backend.py` importing back the 6 its execution orchestration still calls. No behavior change and no test file changes needed — the two existing string-based monkeypatches in `tests/test_crewai_runtime.py` keep working unchanged since the moved functions remain bound in `crewai_backend`'s own module namespace via the import.
- Changed `src/rps/agents/crewai_backend.py` (Phase 3 of the staged module split, see ADR-059): moved the 14 CrewAI agent/crew/LLM construction and tool-resolution functions into a new `src/rps/agents/crewai_builders.py` module, with `crewai_backend.py` importing back the ones its execution orchestration still calls. No behavior change; `tests/test_crewai_runtime.py`'s import block was split for the two directly-imported names (`_emit_crew_task_prepared_events`, `_task_tools_for_blueprint`).
- Changed `src/rps/agents/crewai_backend.py` (Phase 4, the final phase of the staged module split, see ADR-059): moved the 25 season/phase bundle-normalization functions plus two closely-related internal helpers (`_as_int`, `_as_list`) and four exclusive constants into a new `src/rps/agents/crewai_bundle_normalization.py` module, keeping a local `_as_map` copy since it also has 15 other uses that stay in `crewai_backend.py`. No behavior change; `tests/test_crewai_runtime.py`'s import block was split for `_normalize_publication_link`. This closes out ADR-059's active phases — the remaining code in `crewai_backend.py` (contract/context-block building and task-execution orchestration) is excluded from further splitting per the ADR's Exceptions.
- Changed `src/rps/agents/crewai_backend.py` (Phase 5 of the staged module split, see [ADR-060](/doc/adr/ADR-060-crewai-backend-context-and-execution-split.md)): moved the 5 context/contract-block-building functions (Group D) plus `_as_map` (which turned out, on direct verification, to be exclusively used by this group — moved entirely rather than duplicated, unlike Phase 4's `_as_map` which had shared uses) into a new `src/rps/agents/crewai_context_blocks.py` module. `crewai_backend.py` imports back the 3 names its remaining code still calls. No behavior change; `tests/test_crewai_runtime.py`'s import block was split for the 4 directly-imported names. ADR-060 also found and corrected several classification errors from the original ADR-059 estimate and an automated exploration pass — Group D is 5 functions (not 4), and 9 functions previously thought to be uncategorized "other helpers" turned out, via direct call-site verification, to belong to Group E (tracked as Phase 6 next).
- Changed `src/rps/agents/crewai_backend.py` (Phase 6, the final phase of the staged module split, see [ADR-060](/doc/adr/ADR-060-crewai-backend-context-and-execution-split.md)): moved all 23 task-execution-orchestration functions (Group E — including all 4 remaining public entry points) plus 13 exclusive constants and `ROOT`/`logger` into a new `src/rps/agents/crewai_task_execution.py` module. `crewai_backend.py` now retains only a pre-existing, unrelated dead function (`_phase_document_from_bundle`, verified to have zero call sites repo-wide) — effectively dissolving the original 4357-line module down to 25 lines. No behavior change. Updated the 3 external production consumers that import Group E's public entry points directly (`src/rps/evidence/curation.py`, `src/rps/crewai_runtime/flows.py`, `src/rps/agents/runtime.py`) plus `tests/test_crewai_runtime.py`'s import block, its 9 hardcoded monkeypatch strings, and one full-module `sys.modules` mock (`test_runtime_gateway_dispatches_to_crewai_backend`) that all targeted the old module path. Also added `src/rps/agents/crewai_task_execution.py` to `tests/test_skill_references.py::test_runtime_does_not_manually_render_skill_bodies`'s checked-file list, since the runtime code it guards moved there. This closes out the entire ADR-059/ADR-060 `crewai_backend.py` module-split effort.
- Changed `_build_crewai_tooling` (`src/rps/agents/crewai_task_execution.py`) to build each workspace-read tool as a native, multi-parameter `@tool`-decorated function (e.g. `workspace_get_latest(artifact_type: str)`, `workspace_resolve_season_phase(year: int, week: int)`) instead of wrapping every tool behind a single opaque `payload_json: str` parameter parsed by hand with `json.loads`. Prompted by the systematic workspace-tool audit this session: `read_tool_defs()` already declared full, strict JSON Schema (`required`, `enum`) per tool, but it never reached CrewAI's/the LLM's function-calling interface because of the `payload_json` string-wrapping — a missing or misspelled field was only ever caught after the LLM's call, inside the Python handler, as a `KeyError` or a manual `_require_*` check. CrewAI's `@tool` decorator natively supports multi-parameter functions with auto-inferred schemas (confirmed via its docs), so this shifts the missing/malformed-field check left to CrewAI's own binding, before the call reaches this repo's code at all. `workspace_get_input`'s `input_type`/`artifact_type` fields now use `Literal[...]` matching `read_tool_defs()`'s `enum`; its "at least one of these two" cross-field requirement can't be expressed as a plain function signature and remains a runtime check inside the handler, same as before. Updated the "Tool contract" prompt text in `_build_internal_task_description`/`_build_task_description` to describe named-argument calls instead of the `payload_json` convention, and the `_require_artifact_type`/`_require_int` error messages in `workspace_read_tools.py` to match (they no longer literally receive a field called `payload_json`). `read_tool_defs()` itself is no longer consumed by `_build_crewai_tooling` directly, but is kept as the manually-maintained schema `_build_crewai_tooling`'s native signatures are derived from and checked against (see the new `test_build_crewai_tooling_signatures_match_read_tool_defs` test). This is a wire-format change to every workspace tool call CrewAI makes; CrewAI isn't installed locally, so it could only be verified via the new unit tests (`test_build_crewai_tooling_exposes_native_named_parameters`, `test_build_crewai_tooling_signatures_match_read_tool_defs` in `tests/test_crewai_config_and_builders.py`) and needs a real Docker rebuild + `crew.kickoff()` run to confirm in production before relying on it.

### Fixed

- Fixed `typed_output_present` failing every `output_mode: pydantic` task unconditionally — a regression introduced by the previous commit's fix (`c24baad`), caught by a real end-to-end smoke test rather than by the unit tests added alongside it. The guardrail callable CrewAI actually invokes is built by `build_task_guardrail_kwargs` → `_with_guardrail_telemetry` (`src/rps/crewai_runtime/guardrails_utilities.py`), which pre-normalizes the raw `TaskOutput` into a plain dict via `normalize_artifact_candidate_for_task_guardrails`/`_coerce_mapping` *before* calling the guardrail function — the exact same lenient `.pydantic` → `.json_dict` → `.raw` fallback chain the previous fix targeted, just one layer up. `typed_output_present`'s tightened `.pydantic` check then always found `None` on a dict (dicts never have a `.pydantic` attribute), regardless of whether CrewAI's binding had actually succeeded — a live production run showed `season_context_read` (the crew's very first task) failing 3/3 attempts, initially suspected as a `gpt-5.4-nano` capability issue but traced to this wiring bug instead. `_with_guardrail_telemetry` now special-cases `typed_output_present` to receive the raw, unnormalized `TaskOutput`: it checks a property of CrewAI's own binding step, not content shape, so it's categorically different from every other guardrail that benefits from pre-normalization. Added `test_typed_output_present_wired_through_guardrail_wrapper_checks_raw_pydantic`, which exercises the real `build_task_guardrail_kwargs`-constructed callable end-to-end (confirmed to fail without the fix, by temporarily reverting it and re-running) — the previous commit's unit tests called `typed_output_present` directly, bypassing this wrapper entirely, which is exactly why they didn't catch it.
- Closed a shift-left gap: the adversarial "sibling task guardrail passed but `.pydantic` is `None`" regression test existed only for `_assemble_season_plan_draft_bundle` (added reactively after the real production failure below), not for `_assemble_phase_draft_bundle`'s three siblings (`phase_guardrail_band_draft`/`phase_structure_draft`/`phase_preview_draft`). Added the same parametrized coverage for all three, plus a "sibling task did not run at all" variant, to `tests/test_crewai_output_extraction_and_audit.py` — catches this failure class without needing a real CrewAI run. Also confirmed `role_week_load_bands: list[RoleWeekLoadBandModel | str]`'s union type (flagged as a possible complexity/reliability risk) is not a good simplification target: it's a deliberate, already-normalized (`normalize_role_week_load_bands`/`normalize_role_week_load_bands_from_text` in `src/rps/planning/phase_authority.py`) tolerance mechanism for LLM output drift, not incidental complexity.
- Fixed `typed_output_present` (`src/rps/crewai_runtime/guardrails_generic.py`) silently accepting raw/untyped text as "typed output present": it delegated to `_coerce_payload`, which falls back through `.pydantic` → `.json_dict` → `.raw`, so a task whose LLM response failed strict pydantic validation but still had non-empty raw text passed the guardrail without a retry. This was harmless before `FEAT_finalize_manager_synthesis_boundary` (nothing downstream trusted a sibling task's `.output.pydantic` as a hard dependency), but a real end-to-end smoke test of the new Season assembly path surfaced it directly: `season_phase_blueprint_draft`'s guardrail passed despite `.pydantic` being `None`, and `_assemble_season_plan_draft_bundle`'s `_extract_typed_output` then fell back all the way to the crew-level `CrewOutput` (the *final* task's result, not this sibling's), crashing with `'CrewOutput' object has no attribute 'phase_blueprints'` instead of triggering CrewAI's own guardrail retry. `typed_output_present` now checks `.pydantic` directly. Also hardened `_extract_typed_output`/`_extract_structured_output`/`_collect_typed` with a new `allow_crew_result_fallback` flag (default `True`, forced `False` for all sibling-task lookups in `_assemble_season_plan_draft_bundle`/`_assemble_phase_draft_bundle`): a sibling task's own missing typed output now raises a clear, task-named `RuntimeError` instead of silently substituting the crew's final-task result.
...
- Fixed the remaining `_load_latest_payload`-family duplication: a full per-call-site audit (including one previously untracked copy in `src/rps/orchestrator/season_flow.py`) confirmed that `plan_week.py`, `season_flow.py`, and `week_engine.py`'s `_load_latest_optional` are all consumed identically to the already-shared `LocalArtifactStore.load_latest_payload(...)`, and that `plan/season.py`'s variant (no `athlete_id` parameter, existence-check instead of try/except) can also unify onto it. All four now route through `LocalArtifactStore.load_latest_payload(...)`. `week_engine.py`'s `_load_latest_required` (fail-closed, raises `ValueError` for the mandatory `SEASON_PLAN`) is intentionally different and was left as a file-local special case.
- Fixed `tests/test_evidence_library.py` writing to the real tracked `skills/shared/durability-methodology/references/...` files on every run: the 6 tests that mutate library state (`save_core_studies`/`save_applied_sources`/`save_discovery_state`) now request a new `isolated_evidence_library` fixture that redirects all 12 of `src/rps/evidence/library.py`'s path constants to `tmp_path`; the 2 read-only content-invariant tests are unaffected and still validate the real checked-in library. As a side effect, this also fixed the 5 `test_refresh_evidence_library_*` tests that had started failing because the real `discovery_state.json`'s `last_success_at` (from actual scheduled evidence refreshes) had moved past their hardcoded `now=` values — an isolated, unseeded discovery-state file makes `evidence_refresh_due()` reliably report "due" again, independent of wall-clock/real-automation state. Also changed `test_sync_reference_library_outputs_updates_generated_views` to call `sync_reference_library_outputs(rewrite_yaml=False)`, since it doesn't assert on YAML content but was still re-stamping the real files' `generated_at` field as a side effect.
- Fixed `tests/test_skill_references.py::test_skill_references_are_local_to_skill_directory` rejecting legitimate cross-skill composition references (e.g. `skills/shared/load-estimation-core/SKILL.md`, `skills/phase/guardrails-authoring/SKILL.md`) that have organically grown under "Use this skill together with:" sections across several skills. The rule now allows any `skills/`-prefixed reference provided the target file actually exists (still catching real typos/broken cross-refs), and excludes ellipsis-containing tokens (`...`) from candidate paths, fixing 2 false positives where prohibitive prose ("Do not depend on legacy workout prose under `specs/knowledge/_shared/sources/...`") was being misread as a reference.
- Fixed `tests/test_week_plan_edits.py`'s `_sample_week_plan()` fixture missing the `inherited_planning_posture` and `effective_week_constraints` fields that `specs/schemas/week_plan.schema.json` requires, using the same field values already validated in `tests/test_workout_export.py`.
- Fixed `tests/test_plan_pages.py` being a 3949-line, 58-test file mixing unrelated page/orchestration domains: split it into 7 domain-focused files (`test_plan_season_page.py`, `test_plan_hub_page.py`, `test_season_planning_orchestration.py`, `test_plan_week_orchestration.py`, `test_athlete_profile_pages.py` — merged into the pre-existing file of that name rather than colliding with it, `test_performance_report_page.py`, `test_feed_forward_page.py`), each with only the imports/constants its own tests use. No behavior change; all 58 tests moved verbatim. `tests/test_crewai_runtime.py` (201 tests, 8177 lines) was a separate, much larger split tracked as its own future backlog item — now also done (see below).
- Fixed `tests/test_crewai_runtime.py` being a 8177-line, 201-test file mixing unrelated CrewAI-runtime domains: split it into 12 domain-focused files (`test_crewai_config_and_builders.py`, `test_crewai_season_semantics_normalization.py`, `test_crewai_output_extraction_and_audit.py`, `test_crewai_phase_week_review_guardrails.py`, `test_crewai_season_phase_bundle_normalization.py`, `test_crewai_phase_writer_guardrails.py`, `test_crewai_review_readiness_and_load_context.py`, `test_crewai_scenario_profile_quality.py`, `test_crewai_week_planning_guardrails.py`, `test_crewai_runtime_config_and_status.py`, `test_crewai_flow_and_event_orchestration.py`, `test_crewai_multi_output_execution.py`). The 3 substantial CrewAI-mocking helpers (`_install_fake_crewai_events`, `_install_fake_flow_module`, `_install_strict_state_flow_module`) and the small `_set_module_attrs`/`_open_object_schema_findings`/`_season_scenario_item`/`_season_scenarios_payload` helpers were duplicated only into the files that actually use them, matching the `test_plan_pages.py` split's convention — no shared `conftest.py`. No behavior change; all 201 tests moved verbatim and pass unchanged. This closes out the oversized-test-file backlog item entirely.
- Fixed 4 more `KeyError`-prone bare `args["year"]`/`args["week"]` subscripts in `src/rps/tools/workspace_read_tools.py` (`workspace_resolve_season_phase`, `workspace_resolve_phase_range`, `workspace_find_best_phase_artefact`, `workspace_get_phase_context`) — the same bug class as the `workspace_get_latest`/`artifact_type` fix above, found via a systematic audit of every workspace read tool prompted by that incident. New `_require_int(args, key, *, example)` helper raises a clear, example-bearing `ValueError` instead of a bare `KeyError`.
- Fixed `workspace_get_latest`/`workspace_get_version`/`workspace_list_versions` (`src/rps/tools/workspace_read_tools.py`) returning the raw unwrapped artifact payload (or, for `list_versions`, a bare list) on success instead of the `{"ok": True, ...}` envelope the other 9 workspace-read tools use consistently. Beyond the inconsistency itself, this silently defeated `_build_crewai_tooling`'s `_capture_loaded_input` (`src/rps/agents/crewai_task_execution.py`), which only records a tool's result into `loaded_inputs` when `result.get("ok") is True` — these three tools' successful reads were never being captured. All three now return `{"ok": True, **payload}` (or `{"ok": True, "artifact_type": ..., "versions": [...]}` for `list_versions`).
...
- Added spec-closure coverage for canonical snapshot traceability: snapshot builder tests now assert the full canonical `trace_data` / `trace_events` contract across `ATHLETE_STATE_SNAPSHOT`, round-trip save/load coverage now proves canonical snapshot and advisory metadata survives persistence, and advisory-memory regression tests now lock carry-through of the nested Season objective from `SEASON_PLAN.data.season_intent_principles.season_objective`.
- Added a shift-left Season semantic-coherence fix for forbidden-domain prose drift: active Season synthesis guidance now explicitly forbids positive narrative framing for phase-local forbidden intensity domains, season bundle normalization already strips or rewrites that drift at the phase-blueprint boundary, and final `SEASON_PLAN` normalization re-applies the same deterministic legal-domain framing before strict store-time contract validation.
- Added the decision-complete closure of shift-left planning evidence alignment: Season, Phase, and Week now resolve exact `W - 1` evidence through one shared orchestration path, Phase and Week gate on a current previous-week `DES_ANALYSIS_REPORT`, new scope-specific evidence-alignment tasks/skills feed concise planning implications into the real planning chain, and planning snapshots/context blocks now carry previous-week evidence plus evidence-alignment outputs early enough to shape first-pass synthesis.
- Added tighter shift-left Phase-constraint canonicalization: active Phase structure/finalizer guidance now explicitly forbids runtime/process/governance drift inside `PHASE_STRUCTURE.upstream_intent.constraints`, phase bundle normalization now canonicalizes that field before writer handoff using loaded Season authority when available, and final `PHASE_STRUCTURE` normalization now preserves only canonical inherited constraint wording plus narrow residual external facts.
- Added earlier `PHASE_STRUCTURE` constraint hygiene across the Phase chain: active Phase structure/finalizer guidance now explicitly limits `upstream_intent.constraints` to real inherited planning facts, phase bundle normalization now canonicalizes and freezes that field before writer handoff, and final `PHASE_STRUCTURE` normalization now strips runtime/process-rule drift while preserving canonical inherited constraint wording.
- Added a Phase artifact-quality normalization closure: `PHASE_STRUCTURE.upstream_intent.constraints` is now canonicalized against deterministic upstream fact groups so same-fact writer paraphrases do not survive beside injected season/global constraints, while `PHASE_GUARDRAILS` and `PHASE_STRUCTURE` now replace indirect Season `trace_upstream` entries from the real stored upstream metadata so persisted Season lineage keeps canonical `run_id` quality.
- Added `FEAT_season_finalize_raw_bundle_boundary`: `season_plan_finalize` now crosses a raw JSON boundary before strict internal bundle validation, the runtime performs one narrow repo-owned audit-slot coercion between `constraints[]` and `load_governance[]`, and active Season finalizer guidance now frontloads exact slot ownership for governance audit fields such as `cadence_authority_preserved` and `durability_first_respected`.
- Added shared run-event enrichment hardening for Plan Hub and System Status: the shared run-store loader now normalizes `timestamp`/`ts`, limits direct child telemetry merge to the known `_week` and `_phase_bundle` planning children, backfills sparse step metadata from `run.json` / `steps.json`, and renders compact deterministic `details` strings for structured runtime payloads so both pages show the same richer flow/step/detail context.
- Added a deterministic inherited-scenario-contract freeze across the early Phase chain: phase bundle normalization now injects the full exact inherited contract into top-level and nested Phase payloads, `PHASE_STRUCTURE` pre-guardrail normalization now re-projects that contract from deterministic execution context, active Phase bundle/writer task-prompt-skill files now forbid paraphrasing nested contract narrative fields, and new regressions lock the exact-copy behavior plus fail-closed handling when the deterministic contract is missing.
- Added a narrow inherited-scenario-contract drift closure for `PHASE_STRUCTURE`: store-time and pre-guardrail normalization now share one deterministic contract-precedence helper, and mismatch telemetry now preserves compact source diagnostics even when the guardrail reason is truncated.
- Added a targeted Season-scenario cadence frontloading regression closure: recommendation-default cadence anti-collapse is again frontloaded as a canonical top-priority rule across the active task/prompt/skill surfaces, and new regressions now lock the exact wording plus the explicit pass/fail behavior for shared cadence under recommendation context.
...
- Added `FEAT_runtime_flow_listener_adapter_hardening` as the second Wave-2 runtime-cleanup slice: active CrewAI Flow wrappers and the event-listener bridge now construct runtime-loaded third-party classes inside repo-owned adapter code instead of carrying invalid static subclass patterns through active runtime modules.
- Added `FEAT_runtime_compat_boundary_reduction` as the first Wave-2 runtime-cleanup slice: active CrewAI availability probing now lives in a repo-owned `runtime_status` module, while `compat.py` is reduced to a narrow legacy shim.
- Added `FEAT_repo_metadata_trace_workspace_cleanup` as the first delivery slice of the repo-wide Altlastenprogramm, documenting canonical dict-based trace metadata, explicit legacy-read normalization in `local_store`, and renderer-facing display-only trace handling.
...
### Changed
- Changed project package version to `0.20.27` for the Season finalizer audit-slot integrity hardening and the repository workflow/doc-rule cleanup: non-WIP push commits now require canonical docs to be current, explicit SemVer/tagging decisions are documented in the active agent rules, and the repo-local agent entry/rule structure has been simplified around `AGENTS.md`, `.clinerules`, and `.clinerules.d/`.
- Changed project package version to `0.20.25` for the Season finalizer bundle-shape canonicalization fix: recurring singular audit-slot alias drift (`constraint_audit`, `load_governance_audit`) is now repaired before task guardrails and again at the raw JSON fallback boundary, while `season_bundle_integrity` remains unchanged and still fails on real missing `event_priority`, `macrocycle`, or `phase_blueprints` structure.
- Changed project package version to `0.20.24` for the internal Season-finalizer raw-boundary repair: `season_plan_finalize` now crosses a true internal JSON boundary without CrewAI pre-binding `SeasonPlanDraftBundleModel`, repo-owned audit-slot coercion runs before strict bundle validation, and targeted runtime regressions now prove that repairable governance-slot drift no longer fails before the intended coercion step.
- Changed project package version to `0.20.23` for the canonical snapshot-traceability closure: snapshot and advisory builders now write canonical metadata from upstream artifact `meta` at document-build time, advisory memory now carries the real nested Season objective, `SEASON_PLAN` final normalization now replaces indirect Season lineage with real stored upstream `run_id`s, and the related feature doc is marked `Updated` until the scoped `SEASON_PLAN` runtime acceptance audit is completed.
- Changed project package version to `0.20.22` for the shift-left Season forbidden-domain narrative fix: positive `THRESHOLD`/`VO2MAX` prose can no longer survive from Season synthesis into stored phase narratives when those domains are illegal for the phase, while store-side season semantic contract checks remain unchanged and strict.
- Changed project package version to `0.20.21` for the Season evidence-alignment task-graph fix: the Season planning task order now places `season_evidence_alignment` before every early draft and finalizer task that consumes it as formal CrewAI context, so scoped `SEASON_PLAN` runs no longer fail at crew startup with `unknown or later context tasks` while keeping Season on baseline plus previous-week actual/trend only and without introducing Season report autogating.
- Changed project package version to `0.20.20` for the remaining shift-left evidence-alignment closure: `SEASON_SCENARIOS` now fails closed when its required evidence/alignment preparation chain breaks instead of silently degrading to an evidence-free run, and the early Season/Phase/Week draft tasks now consume evidence-alignment outputs explicitly before synthesis while keeping deterministic authority binding.
- Changed project package version to `0.20.18` for the early Phase-constraint frontloading fix: `structure.upstream_intent.constraints` is now classified and canonicalized already at phase bundle normalization, active Phase structure/finalizer files now explicitly exclude runtime/process rules from that field, and final `PHASE_STRUCTURE` normalization remains only the last deterministic safety layer.
- Changed project package version to `0.20.16` for the internal Phase runtime-model contract fix: `PHASE_GUARDRAILS`, `PHASE_STRUCTURE`, and `PHASE_PREVIEW` now use closed structured Pydantic submodels that match the active normalized payload shape, phase bundle review-readiness now treats nested summary blocks as structured objects instead of legacy string lists, and the direct Phase runtime regressions now reject the obsolete list-form payloads that previously caused `phase_preview_draft` to fail at the internal model boundary.
- Changed project package version to `0.20.15` for the remaining Phase inherited-scenario-contract drift closure: `PHASE_STRUCTURE` now resolves inherited scenario contract precedence through one shared helper across pre-guardrail and store-time normalization, and runtime mismatch telemetry now preserves compact source diagnostics instead of truncating them away.
- Changed project package version to `0.20.13` for Season-scenario wording de-anchoring: active `constraint_summary` guidance now uses diversified pattern-based few-shot examples with explicit anti-mechanical-copy instructions, reducing literal phrase anchoring while preserving concrete positive authoring guidance.
- Changed project package version to `0.20.14` for shared run-event enrichment hardening: Plan Hub and System Status now normalize `timestamp`/`ts` centrally, merge only the known direct planning child runs, backfill sparse parent step metadata from run records, and render deterministic compact `details` text for structured runtime payloads.
- Changed project package version to `0.20.12` for the Phase inherited-scenario-contract freeze fix: phase bundle normalization now injects the exact inherited scenario contract before review/writer handoff, `PHASE_STRUCTURE` pre-guardrail normalization re-projects the same contract from deterministic execution context, and active Phase authoring files now explicitly forbid paraphrasing nested inherited-contract narrative fields.
- Changed project package version to `0.20.11` for the remaining Phase artifact normalization fix: `PHASE_PREVIEW` now deterministically restores exact preview week keys from the shared skeleton, direct `PHASE_STRUCTURE -> PHASE_GUARDRAILS` lineage now replaces synthetic intermediate run ids with the real stored upstream run id before final trace canonicalization, and the internal `PHASE_PREVIEW` payload model now matches the real structured week-agenda schema.
- Changed project package version to `0.20.10` for the typed outer-flow bootstrap registration fix: Phase/Week/Report/Feed-Forward/Coach CrewAI wrappers now register start/listener methods under names that match their dynamic flow keys, so typed `Flow[StateModel]` runs no longer fail at kickoff with `KeyError: 'bootstrap'`.
- Changed project package version to `0.20.7` for the CrewAI outer-flow state regression fix: dynamically built Season/Phase/Week/Report/Feed-Forward/Coach wrappers now preserve typed `Flow[StateModel]` bases instead of falling back to dict-backed proxy state.
- Changed project package version to `0.20.6` for the pipeline metadata backfill bugfix: data-pipeline artefacts that already carry sufficient envelope metadata can now receive a canonical `meta.version_key` during canonicalization instead of failing late in schema validation.
- Changed project package version to `0.20.5` for the first Wave-4 prompt/skill/doc cleanup slice: active Season/Phase skill variables are now explicitly defined/mapped, the shared durability skill is self-contained under the current active-layer standard, and selected planner/finalizer/review surfaces now explicitly reject historical migration docs and superseded prose as operative runtime authority.
- Changed project package version to `0.20.4` for the Wave-3 full-typecheck closure slice: full-repo mypy is now green again, active typing debt in planning/runtime/store helpers and high-friction test harnesses was closed without relaxing repo-wide typing rules, and exact-range workspace discovery now stays correct immediately after `save_document(...)` writes.
- Changed project package version to `0.20.3` for the second Wave-2 runtime-boundary hardening slice: active CrewAI flow wrappers now create dynamic Flow classes at the adapter boundary, the CrewAI event listener bridge now dynamically constructs its runtime-loaded base class locally, and adjacent runtime helpers were tightened so the active CrewAI adapter surface passes curated type checking without broadening repo-wide `Any` usage.
- Changed project package version to `0.20.2` for the Wave-2 runtime-boundary cleanup: active runtime/orchestrator/chat/binding entrypoints no longer import `crewai_runtime.compat` directly for CrewAI availability checks, and the compatibility module now only re-exports the canonical runtime-status helper.
- Changed project package version to `0.20.1` for the Wave-1 metadata/trace cleanup: repo-internal workspace and CrewAI artifact metadata types now treat `trace_upstream`/`trace_data`/`trace_events` as canonical dict references, local-store write paths normalize trace fields before persistence, legacy string trace entries remain read-compatible through explicit normalization only, and renderer documentation now reflects display-ready trace formatting instead of mixed raw trace shapes.
- Changed project package version to `0.20.0` to capture the remaining audit-closure patch: `WEEK_PLAN` now persists `effective_week_constraints` separately from scenario posture, week objectives now describe the actual planned load inside the active binding band, governed artifacts now persist canonical `meta.version_key` plus cleaner direct upstream lineage, and housekeeping now scans only real athlete workspaces instead of creating pseudo-athlete runs such as `athlete=runs`.
- Changed project package version to `0.19.20` to capture the generic season/preview consistency follow-up: `SEASON_PLAN.season_load_envelope.expected_average_weekly_kj_range` now derives from authoritative role-week load bands instead of broad phase corridors, `PHASE_PREVIEW` is normalized and rendered consistently as an informational shared-skeleton view, active preview guidance no longer frames Preview as binding week authority, and final Phase trace lineage now deduplicates immediate upstream references in favor of canonical in-process entries.
- Changed project package version to `0.19.19` to capture shared run-event telemetry enrichment: Plan Hub and System telemetry tables now merge direct child run events, historical parent `STEP_*` rows are deterministically backfilled from `steps.json`, and future worker-emitted `RUN_*` / `STEP_*` payloads now persist richer `step` / `details` / `agent` / `writes` / `authority` context for clearer runtime inspection.
- Changed project package version to `0.19.17` to capture the Plan Hub runtime-progress visibility improvement: active runs now show pipeline `x/y` progress plus current flow, crew, task, task-progress, agent, and model details derived from existing runtime events, and the run event table now exposes crew/agent/progress/model/component columns directly.
- Changed project package version to `0.19.16` to capture the `PHASE_GUARDRAILS` persisted-band boundary fix: exact role-week phase authority now serializes deterministically into schema-valid persisted weekly bands with canonical `band.notes`, downstream phase artifacts mirror the persisted shape instead of leaking internal top-level `role`, and scoped Phase orchestration no longer reports failed forced-step runs as completed.
- Changed project package version to `0.19.15` to capture the remaining Phase writer-path hardening: `PHASE_GUARDRAILS` now uses deterministic pre-guardrail normalization, downstream Phase artifacts now repair immediate `trace_upstream` lineage in final normalization, and supported `quality_intent` fields are canonicalized to stabilization semantics for the active shortened re-entry phase family.
- Changed project package version to `0.19.14` to capture the `phase_bundle_finalize` contract-rediscovery fix: the Phase finalizer now loses contract-read tools when deterministic phase authority is already injected, receives a compact authority-freeze block in its task input, and the active finalizer task/prompt/skill guidance now explicitly treats contract tools as fallback-only for missing authority fields.
- Changed project package version to `0.19.13` to capture the Phase writer exact-authority frontloading fix: `PHASE_STRUCTURE` pre-guardrail normalization now projects exact legality, forbidden domains, modalities, and phase-local objective primarily from bound `phase_execution_context`, writer task input now includes compact exact-authority blocks for Phase artifacts, and active Phase task/prompt/skill files now frontload field-level copy rules and canonical examples instead of relying on abstract authority wording.
- Changed project package version to `0.19.12` to capture the operational-`NONE` Phase authority fix, keeping `PHASE_STRUCTURE.allowed_intensity_domains` as exact persisted phase legality while treating `REST -> NONE/NONE` and fixed non-training-day `NONE/NONE` as operational Preview/store semantics rather than structural legality widening.
- Changed project package version to `0.19.11` to capture deterministic Phase bundle nested-intent normalization, so `phase_bundle_finalize` rewrites internal nested `phase_intent` fields to the canonical phase token before review-readiness validation and fails closed during normalization when canonical intent is unavailable.
...
- Changed the active `season_scenarios` task/prompt/skill contract to positively specify the remaining required narrative fields as well, with concrete examples for scenario promise, load philosophy, risk profile, differentiators, week feel, payoff/cost, event alignment, constraints, KPI notes, decision notes, assumptions, unknowns, and top-level notes.
...
### Fixed
- Fixed the remaining Phase artifact quality defects after successful scoped runs: stored `PHASE_PREVIEW.weekly_agenda_preview[*].week` no longer drops out during normalization, direct `PHASE_STRUCTURE.meta.trace_upstream` now points to the real stored `PHASE_GUARDRAILS.run_id` instead of a synthetic intermediate id, and preview writer/runtime normalization now fail closed when weekly preview coverage no longer matches exact shared-skeleton week coverage.
- Fixed the typed CrewAI outer-flow bootstrap/listener registration regression: dynamic Phase/Week/Report/Feed-Forward/Coach wrappers now use consistent decorated method names for `bootstrap` and downstream listeners, and strict typed-flow regression tests now cover these wrappers so kickoff failures like `KeyError: 'bootstrap'` cannot silently reappear.
- Fixed the CrewAI outer-flow state regression introduced by the recent adapter hardening: dynamically generated outer-flow classes now retain the specialized typed Flow base, so runtime state assignment like `flow.state.athlete_id` no longer crashes Season planning with a dict-backed proxy state.
- Fixed the Intervals refresh `ZONE_MODEL` regression: pipeline envelope canonicalization now derives missing `meta.version_key` values from existing authoritative metadata, so zone-model writes no longer fail solely because the payload omitted that required meta field.
- Fixed exact-range workspace visibility after `save_document(...)`: `index.json` writes now preserve string `iso_week` / `iso_week_range` metadata instead of only dict-shaped values, so freshly written Phase artefacts are immediately discoverable through `IndexExactQuery` and scoped plan-week runs no longer misreport those artefacts as missing.
- Fixed the remaining `PHASE_GUARDRAILS` first-pass writer drift: pre-guardrail normalization now freezes exact role-week load bands, exact legality, and phase-local objective from deterministic phase execution authority before CrewAI task guardrails run, instead of letting S5-shaped draft output trigger avoidable retry failures.
- Fixed Phase binding-artifact traceability so normalized `PHASE_STRUCTURE` now carries the exact stored `PHASE_GUARDRAILS` in `meta.trace_upstream`, and normalized `PHASE_PREVIEW` now carries the exact stored `PHASE_STRUCTURE` immediate upstream lineage instead of leaving binding metadata empty.
- Fixed residual Phase semantic drift so supported `BASE / shortened_re_entry` artifacts now canonicalize `quality_intent` to `Stabilization` instead of preserving free-text `Build` wording in downstream serialized fields.
- Fixed the long-running `phase_bundle_finalize` contract-rediscovery loop: when deterministic `phase_execution_context` and `phase_slot_context` are already injected, the finalizer no longer gets live access to the same contract-read tools, so it must synthesize from bound authority instead of repeatedly reloading phase execution and slot contracts.
- Fixed the remaining `PHASE_STRUCTURE` writer guardrail drift against exact phase legality: pre-guardrail Phase writer normalization now prefers bound deterministic `phase_execution_context` over optional `season_plan` reads, `PHASE_PREVIEW` stays aligned with the same downstream structure-authority chain, and compact legality diagnostics now distinguish missing authority sources from raw model drift when `phase_structural_allowed_domains_mismatch` still fires.
- Fixed the remaining `PHASE_STRUCTURE` / `PHASE_PREVIEW` operational-`NONE` leak: structure normalization no longer appends `NONE` into exact phase legality, guarded-store preview validation now accepts `NONE` only for `REST` or fixed non-training days, and contradictory tests were aligned to the exact-structural-legality contract.
- Fixed `phase_bundle_finalize` nested-intent drift: deterministic phase-bundle normalization now rewrites nested internal `guardrails.phase_intent`, `structure.phase_intent`, and `preview.phase_intent` to the canonical phase-intent token from `phase_execution_context`, and fails closed during normalization when that canonical intent is unavailable.
- Fixed the remaining Phase writer guardrail ordering defect: `PHASE_STRUCTURE` and `PHASE_PREVIEW` writer-task candidates are now deterministically pre-normalized against exact persisted Phase authority before CrewAI task guardrails run, so exact legality, exact weekly kJ bands, phase-local objective, and shared preview skeleton semantics are validated on the same projected candidate that guarded-store persistence already enforces.
...
- Season, Phase, and Week active planner/review/writer guidance now follow an explicit source-first 3-pass model with Pass 3 self-audit checklists, explicit Pass 1 vs Pass 2 loopback classification, formal review confirmation language, and writer preconditions that keep semantic repair upstream.
