...
### Changed
- Changed project package version to `0.19.10` to capture the Phase writer-task pre-normalization fix, so `PHASE_STRUCTURE` and `PHASE_PREVIEW` validate against exact persisted authority before CrewAI task guardrails run while guarded-store normalization remains the second safety layer.
