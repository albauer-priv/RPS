...
Purpose
This chapter defines how to produce **schema‑valid BLOCK_EXECUTION_ARCH JSON**.

---

## ARTIFACT: BLOCK_EXECUTION_ARCH
...
- `temporal_scope` MUST be copied from an upstream artefact (prefer stored BLOCK_GOVERNANCE for
  the same range; otherwise use the Macro Overview phase `date_range`). **Do NOT compute dates.**
...
- `source` (string; e.g., filename of block_governance)
...
- Validate against `block_execution_arch.schema.json` before calling the store tool.
...
- Validate against schema before calling `store_block_execution_arch`.
...
- STOP if producing `BLOCK_EXECUTION_ARCH` and `data.self_check.no_numeric_target_introduced`
  is missing or false.
- STOP if `load_ranges.source` is not exactly `block_governance_YYYY-WW.json`
...
### EXAMPLE: BLOCK_EXECUTION_ARCH (minimal valid)
...
```json
{
  "meta": {
    ...
    "authority": "Binding",
    "owner_agent": "Meso-Architect",
    "run_id": "example_block_execution_arch_2026_w04",
  ...
  "data": {
    ...
    "load_ranges": {
      ...
      "source": "block_governance_2026-04.json"
    },
    ...
    "relationships": {
      ...
      "does_not_replace": ["block_governance"]
    },
    "self_check": {
