...
from rps.crewai_runtime.flows import run_feed_forward_flow
...
def run_feed_forward_chain(
    ...
    *,
    ...
    def _run_phase_feed_forward() -> JsonMap:
        ...
        season_ff_version_local = store.resolve_week_version_key(
            athlete_id,
            ArtifactType.SEASON_PHASE_FEED_FORWARD,
            selected_week_key,
        )
        season_phase_ref = (
            f"season_phase_feed_forward_{season_ff_version_local}.json"
            if season_ff_version_local
            else ""
        )
        planning_context_snapshot_local = save_planning_context_snapshot(
            store,
            athlete_id,
            target_week=target_week,
            phase_info=phase_info,
        ...
        planning_context_snapshot_block_local = build_planning_context_snapshot_prompt_block(
            planning_context_snapshot_local
        )
        save_advisory_memory(
            store,
            athlete_id,
        ...
        return run_agent_multi_output(
            ...
            user_input=(
                ...
                f"{athlete_state_snapshot_block}\n"
                f"{planning_context_snapshot_block_local}\n"
                f"{des_context_block}\n\n"
    ...
    phase_ff_result = flow_results["phase_result"]
    if not phase_ff_result.get("ok"):
        return FeedForwardChainResult(
            ok=False,
            report_ok=True,
    ...
    save_advisory_memory(
        ...
        season_plan_payload=_as_map(season_plan_payload),
        des_analysis_payload=selected_report_payload or {},
        season_phase_feed_forward_payload=final_selected_season_phase_ff_payload or {},
