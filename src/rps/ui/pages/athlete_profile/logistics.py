...
with st.expander("Export / Import", expanded=False):
    _export_bytes = export_input(store, athlete_id, ArtifactType.LOGISTICS)
    if _export_bytes:
        st.download_button(
            "Download as JSON",
            data=_export_bytes,
            file_name=suggest_export_filename(athlete_id, ArtifactType.LOGISTICS),
    ...
    _uploaded = st.file_uploader("Import from file", type=["json"], key="import_logistics")
    if _uploaded is not None:
        if st.button("Save imported input", key="save_import_logistics"):
            try:
                _vk = import_input(store, athlete_id, ArtifactType.LOGISTICS, _uploaded.getvalue())
                st.success(f"Imported and saved (version: {_vk}). Review form above.")
                set_status(status_state="done", title="Logistics", message="Imported input from file.")
            except ValueError as exc:
                st.error(f"Import failed: {exc}")
                set_status(status_state="error", title="Logistics", message="Import failed.")
