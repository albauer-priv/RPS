...
- [ ] FEAT_crewai_backend_module_split — split `src/rps/agents/crewai_backend.py` (4357 lines, 87 top-level functions spanning normalization, task/crew building, execution, and persistence) into focused modules; needs an ADR first per `.clinerules.d/10-docs-specs-adr.md` since it touches cross-cutting contracts.
...
- [ ] Audit and consolidate the remaining `_load_latest_payload`-family variants: `src/rps/orchestrator/plan_week.py` (returns `None` instead of `{}` on non-dict success), `src/rps/ui/pages/plan/season.py` (no `athlete_id` parameter, existence-check instead of try/except), and `src/rps/planning/week_engine.py` (`_load_latest_required` raises vs `_load_latest_optional`). These diverge in real behavior, not just shape, so each call site needs individual verification before merging — do not do this as one blind pass.
- [ ] Also consolidate the still-duplicated `_load_selected_week_artifact`-family helpers across the same files (separate from `_load_latest_payload`, not yet investigated for behavioral divergence).
- [ ] Split the two oversized test files `tests/test_crewai_runtime.py` (8172 lines) and `tests/test_plan_pages.py` (3941 lines) by domain to reduce navigation/merge friction.
