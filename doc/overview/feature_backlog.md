...
- [ ] Consolidate the repeated `_load_latest_payload`/`_load_selected_week_artifact`-family helpers duplicated across `src/rps/ui/pages/coach.py`, `src/rps/ui/pages/plan/season.py`, `src/rps/orchestrator/plan_week.py`, `src/rps/orchestrator/advisory_actions.py`, and `src/rps/planning/week_engine.py` behind one shared workspace-level helper.
