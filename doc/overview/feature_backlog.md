...
10) ~~**FEAT_workout_editor_agenda_adjustments**~~ — **done** — bounded agenda-level edits (planned_kj, planned_duration, day_role) via Coach tool without re-planning.

## Implemented / In-Progress

- [x] FEAT_workout_editor_agenda_adjustments — bounded agenda-level edits (planned_kj, planned_duration, day_role) for one agenda day: preview_update_agenda_day() in week_plan_edits.py + Coach tool + 8 tests.
...
- [x] FEAT_workout_editor_agenda_adjustments — support bounded agenda-level edits such as `planned_kj`, `planned_duration`, and selected day-role adjustments without full week re-plan.
...
- [x] FEAT_auto_retry_transient — auto-retry transient failures with clear logs.
- [x] FEAT_season_scenarios_context_injection — Season Scenarios moved to injected-context pattern: pre-loaded `athlete_profile`, `planning_events`, `logistics`, `availability`, `kpi_profile` injected directly into task description; workspace tool calls removed; semi-static inputs placed before dynamic context blocks for provider prefix caching.
...
- [x] FEAT_vo2_scenario_activation — activate `ceiling_first_durability` season archetype when athlete profile contains VO2max development objectives: scenario-generation skill MUST produce at least one ceiling-first scenario (VO2MAX in early phases) when runway ≥ 20 weeks; macrocycle-architecture skill now mandatorily maps P01–P02 to `vo2_build` intent when scenario uses the archetype.
- [x] FEAT_seasonal_availability — extend AVAILABILITY schema with optional `seasonal_context` block (outdoor/indoor months, indoor max session hours); add Streamlit form section; inject into season orchestrator context; update scenario and macrocycle skills to describe winter/indoor phases with appropriate load ceiling and intensity density.
...
- [x] FEAT_guardrails_module_split — split `src/rps/crewai_runtime/guardrails.py` (2346 lines) into 8 focused modules under `src/rps/crewai_runtime/`, per `doc/adr/ADR-061-crewai-guardrails-module-split.md` — all 7 phases complete. `guardrails_context.py` (ContextVar mechanism + type aliases), `guardrails_generic.py`/`guardrails_schema.py` (generic + artifact-envelope validators), `guardrails_utilities.py` (payload coercion, diagnostics, telemetry wrapper, context accessors), `guardrails_phase.py`/`guardrails_week.py`/`guardrails_season.py` (domain validators, `guardrails_season.py` including the 440-line `season_scenarios_profile_quality`), `guardrails_registry.py` (`REGISTRY` + resolution functions). `guardrails.py` itself removed (matching `crewai_backend.py`'s full retirement after ADR-059/060) — a relative-import consumer in `src/rps/crewai_runtime/bindings.py` an earlier absolute-path grep missed was caught and repointed. All 41 guardrail names in `config/crewai/task_policies.yaml` verified to resolve against the new 43-entry `REGISTRY`. Full details: `doc/specs/features/FEAT_guardrails_module_split.md`.
...
- [x] FEAT_finalize_manager_synthesis_boundary — supersedes FEAT_season_finalize_raw_bundle_boundary.
- [x] FEAT_scenario_prose_quality — multi-round improvement of scenario generation skill quality: (1) v0.35.0: `ceiling_first_durability` mandate moved to Hard rules with pre-return self-check and escape-hatch removal; CRITICAL EXCEPTION separating VO2max development objective from competitive-ambition mismatch. (2) v0.36.0: field completion contract rewritten with concrete ELI5 GOOD/BAD examples, structured-to-narrative field connection rule. (3) v0.37.0–v0.37.1: all examples replaced with mandatory fill-in-the-blanks prose templates per field (`typical_week_feel`, `load_philosophy`, `core_idea`, `risk_profile`, `key_differences`, `constraint_summary`, etc.) with generic slot labels ("primary long session day", "shorter session days") that work for any athlete's availability structure, not just a specific day layout.
- [x] FEAT_evidence_library_activation_integration — integrated four newly activated study groups into `evidence_layer_durability.md`: Maunder et al. (2021) `dur_core_001` as the formal peer-reviewed durability definition; detraining studies `dur_core_026/027/028` (Coyle 1986, Mujika & Padilla 2000) as new Section 6 "Training Continuity & Detraining Risk"; carbohydrate studies `dur_core_030/032/033/036` (Jentjens & Jeukendrup 2005, Rowlands 2015, Jeukendrup 2014, Viribay 2020) as new fueling subsection with duration-anchored dosing guidance. Evidence → Principle mapping updated.
- [x] FEAT_evidence_discovery_query_fix — tightened all seven `DISCOVERY_TOPICS` PubMed queries in `src/rps/evidence/refresh.py` to require explicit sports/exercise context (`[tiab]` field tags or `[MeSH Terms]`); bare queries matched materials-science "cycling" (battery charge cycles) and "durability" (material property), producing 51 irrelevant false-positive entries in the Sep 14 refresh. All 51 `dur_auto_2026_*` entries marked `rejected`; documented in `FEAT_evidence_curation_pipeline.md` Section 13. Redesigned `season_plan_finalize`/`phase_bundle_finalize` so the manager LLM no longer reproduces fields already produced by already-typed same-crew sibling tasks (`constraints[]`/`load_governance[]`/`phase_blueprints` for Season; `guardrails`/`structure`/`preview` for Phase); repo code assembles the full bundle deterministically post-`kickoff()` instead. Fixes the root cause of a production `season_bundle_audit_slot_integrity` failure (3 wasted LLM calls) rather than the symptom the superseded FEAT patched. Added typed `WeekPlanningNoteModel` for Week's previously-untyped narrative-note fields.
